...
@frappe.whitelist()
def store_report_as_excel(filters=None):
    ...
    writer.writerow([col.get("label") for col in columns])  # headers
    for row in data:
        writer.writerow([row.get(col.get("fieldname"), "") for col in columns])

    file_content = output.getvalue().encode("utf-8")
    filename = "Supplier_Quotation_Comparison.csv"

    # Step 1: Create a new Supplier Quotation Comparison document
    comparison_doc = frappe.get_doc({
        "doctype": "Supplier Quotation Comparison"
    })
    comparison_doc.insert(ignore_permissions=True)

    # Step 2: Attach the file to that document
    ...
    # Step 3: Update the attachment field
    frappe.db.set_value(
        "Supplier Quotation Comparison",
        comparison_doc.name,
        "quotation_comparison_attachment",
        attached_file.file_url
    )

    frappe.msgprint(f"Report stored in Supplier Quotation Comparison: <a href='/app/supplier-quotation-comparison/{comparison_doc.name}'>{comparison_doc.name}</a>")
