...
frappe.query_reports["DT - Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
	...
	onload: (report) => {
		...
		report.page.add_inner_button(__('Store'), function() {
            frappe.call({
                method: "dt_brightlifecare_customization.dt_brightlifecare_customization.report.dt___supplier_quotation_comparison.dt___supplier_quotation_comparison.store_report_as_excel",
                args: {
                    filters: report.get_values()
                },
                callback: function(r) {
                    if (!r.exc) {
                        frappe.msgprint(__('Report has been stored as an attachment.'));
                    }
                }
            });
        });
